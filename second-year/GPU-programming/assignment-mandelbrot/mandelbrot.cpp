...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < 20; i++) {
    result[i] = get_time_for_creation(X0, Y0, X1, Y1, POZ, PION, ITER, Iters) /
                CLOCKS_PER_SEC * 100 * 1000;
...
void makePicture(int* Mandel, int width, int height, int MAX) {
  ...
  for (int j = height - 1; j >= 0; j--) {
    for (int i = 0; i < width; i++) {
      ...
      int indx =
          (int)floor(5.0 * scale * log2f(1.0f * Mandel[j * width + i] + 1));
...
void makePictureInt(int* Mandel, int width, int height, int MAX) {
  ...
  int MyPalette[35][3] = {
      {255, 0, 255}, {248, 0, 240}, {240, 0, 224}, {232, 0, 208}, {224, 0, 192},
      {216, 0, 176}, {208, 0, 160}, {200, 0, 144}, {192, 0, 128}, {184, 0, 112},
      {176, 0, 96},  {168, 0, 80},  {160, 0, 64},  {152, 0, 48},  {144, 0, 32},
      {136, 0, 16},  {128, 0, 0},   {120, 16, 0},  {112, 32, 0},  {104, 48, 0},
      {96, 64, 0},   {88, 80, 0},   {80, 96, 0},   {72, 112, 0},  {64, 128, 0},
      {56, 144, 0},  {48, 160, 0},  {40, 176, 0},  {32, 192, 0},  {16, 224, 0},
      {8, 240, 0},   {0, 0, 0}};
