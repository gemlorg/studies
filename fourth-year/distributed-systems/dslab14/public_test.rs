...
pub(crate) mod tests {
    ...
    struct FechChordNodeRoutingState {
        reply_sender: UnboundedSender<ChordRoutingState>,
    }

    #[async_trait::async_trait]
    impl Handler<FechChordNodeRoutingState> for ChordNode {
        async fn handle(&mut self, _self_ref: &ModuleRef<Self>, msg: FechChordNodeRoutingState) {
            msg.reply_sender.send(self.fetch_routing_state()).unwrap();
        }
    }
    ...
    struct ReplaceChordNodeRoutingState {
        ...
        ack_sender: UnboundedSender<()>,
    }

    #[async_trait::async_trait]
    impl Handler<ReplaceChordNodeRoutingState> for ChordNode {
        async fn handle(&mut self, _self_ref: &ModuleRef<Self>, msg: ReplaceChordNodeRoutingState) {
            self.replace_routing_state(msg.rs);
            msg.ack_sender.send(()).unwrap();
        }
    }
