...
// another thread, depending on the runtime type).
// Spawning tasks is an important concept in Tokio:
#[tokio::main]
async fn tokio_spawning_tasks() {
    // When we create a new future, it doesn't execute any code yet.
    let future = async {
        tokio::time::sleep(Duration::from_millis(200)).await;
        println!("I am inside the spawned task.");
        42
    };
    // We can spawn a new task "in the background":
...
// Here is a TCP example (handling UDP is similar):
#[tokio::main]
...
// This example shows how spawning allows parallelism.
#[tokio::main]
async fn tokio_concurrency_vs_parallelism() {
    // Let's make a future builder that simulates actual computation.
    let make_future = || async {
        tokio::time::sleep(Duration::from_millis(200)).await;
        println!(
            "Hello from the other side. \
            Now we're going to compute the millionth digit of π!"
        );
        std::thread::sleep(Duration::from_millis(33)); // shhh...
        println!("5 or 1, depending on how you count.");
        42
    };

    let fut_1 = make_future();
    let fut_2 = make_future();

    // We created two futures that will run concurrently, but not in parallel.
    // That is, they are executed in the same greenlet.
    tokio::time::sleep(Duration::from_millis(100)).await;
    // They haven't started executing yet, even though we yielded above.
    // The code will print about 266 every time.
    let start = tokio::time::Instant::now();
    let _ = tokio::join!(fut_1, fut_2);
    let end = tokio::time::Instant::now();

    println!("Join of futures took {:?}", end - start);

    let task_1 = tokio::spawn(make_future());
    let task_2 = tokio::spawn(make_future());

    // Note: we created two greenlets that may be scheduled to separate threads,
    // therefore, they may run in parallel.
    tokio::time::sleep(Duration::from_millis(100)).await;
    // Depending on the scheduler, you may see 133 or 166.
    let start = tokio::time::Instant::now();
    let _ = tokio::join!(task_1, task_2);
    let end = tokio::time::Instant::now();

    println!("Join of tasks took {:?}", end - start);
}
...
fn main() {
    ...
    tokio_network();
    tokio_sync();
    tokio_concurrency_vs_parallelism();
}
