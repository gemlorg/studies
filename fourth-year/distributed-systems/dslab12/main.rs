...
impl<const N: usize> Handler<InitClient<N>> for SimpleClient<N> {
    async fn handle(&mut self, _self_ref: &ModuleRef<Self>, msg: InitClient<N>) {
...
impl<const N: usize> Handler<InternalClientCommand> for SimpleClient<N> {
    async fn handle(&mut self, _self_ref: &ModuleRef<Self>, msg: InternalClientCommand) {
        match msg.action {
            Action::Insert { idx, .. } => {
                debug!("Client {}: Inserting at {}", self.rank, idx);
                assert!(idx <= self.text.len(), "Invalid idx!");
            }
            Action::Delete { idx } => {
                debug!("Client {}: Deleting at {}", self.rank, idx);
                assert!(idx < self.text.len(), "Invalid idx!");
            }
            Action::Nop => {
...
impl<const N: usize> Handler<Edit> for SimpleClient<N> {
    async fn handle(&mut self, _self_ref: &ModuleRef<Self>, msg: Edit) {
...
impl<const N: usize> Handler<InitBroadcast<N>> for SimpleBroadcast<N> {
    async fn handle(&mut self, _self_ref: &ModuleRef<Self>, msg: InitBroadcast<N>) {
