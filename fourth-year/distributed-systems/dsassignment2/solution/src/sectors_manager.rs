...
pub(crate) mod sectors_manager_impl {
    ...
    async fn restore_map(path: PathBuf) -> Arc<RwLock<HashMap<SectorIdx, MetaData>>> {
        ...
        for entry in dir {
            ...
            if entry_path.extension().is_some() {
                ...
                {
                    // can be two, the older(lower timestamp) will be overwritten
                    match map.get(&meta.id) {
                        Some(old_meta) => {
                            if old_meta.timestamp < meta.timestamp {
                                tokio::fs::remove_file(path.clone().join(old_meta.as_string()))
                                    .await
    ...
    impl SectorsManager for SectorsManagerImpl {
        async fn read_data(&self, idx: SectorIdx) -> SectorVec {
            ...
            match meta {
                Some(meta) => {
                    let data = match tokio::fs::read(self.extend_path(meta.as_string())).await {
                        Ok(data) => data,
        ...
        async fn write(&self, idx: SectorIdx, sector: &(SectorVec, u64, u8)) {
            ...
            let path = self.extend_path(meta.as_string());
            ...
            if let Some(old_meta) = self.map.write().await.get(&idx) {
                tokio::fs::remove_file(self.extend_path(old_meta.as_string()))
                    .await
                    .expect("Error removing old file");
            }
