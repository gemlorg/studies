; ModuleID = 'program'


 


declare external ccc  void @printInt(i32)    


declare external ccc  void @printString(i8*)    


declare external ccc  void @error()    


declare external ccc  i32 @readInt()    


declare external ccc  i8* @readString()    


declare external ccc  i8* @_concatStrings(i8*, i8*)    


declare external ccc  i32 @_compareStrings(i8*, i8*)    


declare external ccc  i8* @_malloc(i32)    


declare external ccc  i32 @_count_arr_length(i8*, i32)    


@_g0 =    constant [1 x i8] c"\00"


%_arr = type {i8*, i32}


define external ccc  %_arr* @__arr_malloc()    {
entry:
  %0 = getelementptr inbounds %_arr, %_arr* zeroinitializer, i32 1 
  %1 = ptrtoint %_arr* %0 to i32 
  %2 =  call ccc  i8*  @_malloc(i32  %1)  
  %3 = bitcast i8* %2 to %_arr* 
  ret %_arr* %3 
}


%A = type {%_A_vtype*, i32, i32, i8*, i8*}


%_A_vtype = type {void (%A*)*}


@_A_vdata =    global %_A_vtype { void (%A*)* @_cf_A_f }


define external ccc  %A* @_A_malloc()    {
entry:
  %0 = getelementptr inbounds %A, %A* zeroinitializer, i32 1 
  %1 = ptrtoint %A* %0 to i32 
  %2 =  call ccc  i8*  @_malloc(i32  %1)  
  %3 = bitcast i8* %2 to %A* 
  ret %A* %3 
}


%B = type {%_B_vtype*, %A*}


%_B_vtype = type {}


@_B_vdata =    global %_B_vtype {  }


define external ccc  %B* @_B_malloc()    {
entry:
  %0 = getelementptr inbounds %B, %B* zeroinitializer, i32 1 
  %1 = ptrtoint %B* %0 to i32 
  %2 =  call ccc  i8*  @_malloc(i32  %1)  
  %3 = bitcast i8* %2 to %B* 
  ret %B* %3 
}


define external ccc  void @_cf_A_f(%A*  %_this)    {
entry:
  %_v0 = alloca %A* 
  store   %A* %_this, %A** %_v0  
  %_v2 = load   %A*, %A** %_v0  
  %_v1 = getelementptr  %A, %A* %_v2, i32 0, i32 1 
  store   i32 42, i32* %_v1  
  %_v4 = load   %A*, %A** %_v0  
  %_v3 = getelementptr  %A, %A* %_v4, i32 0, i32 1 
  %_v5 = load   i32, i32* %_v3  
   call ccc  void  @printInt(i32  %_v5)  
  ret void 
}


define external ccc  i32 @main()    {
entry:
  %_v4 = bitcast [1 x i8]* @_g0 to i8* 
  %_v6 = bitcast [1 x i8]* @_g0 to i8* 
  %_v0 =  call ccc  %A*  @_A_malloc()  
  %_v1 = getelementptr  %A, %A* %_v0, i32 0, i32 0 
  store   %_A_vtype* @_A_vdata, %_A_vtype** %_v1  
  %_v2 = getelementptr  %A, %A* %_v0, i32 0, i32 1 
  store   i32 0, i32* %_v2  
  %_v3 = getelementptr  %A, %A* %_v0, i32 0, i32 2 
  store   i32 0, i32* %_v3  
  %_v5 = getelementptr  %A, %A* %_v0, i32 0, i32 3 
  store   i8* %_v4, i8** %_v5  
  %_v7 = getelementptr  %A, %A* %_v0, i32 0, i32 4 
  store   i8* %_v6, i8** %_v7  
  %_v8 = getelementptr  %A, %A* %_v0, i32 0, i32 0 
  %_v9 = load   %_A_vtype*, %_A_vtype** %_v8  
  %_v10 = getelementptr  %_A_vtype, %_A_vtype* %_v9, i32 0, i32 0 
  %_v11 = load   void (%A*)*, void (%A*)** %_v10  
   call ccc  void  %_v11(%A*  %_v0)  
  ret i32 0 
}


define external ccc  %A* @newA()    {
entry:
  %_v4 = bitcast [1 x i8]* @_g0 to i8* 
  %_v6 = bitcast [1 x i8]* @_g0 to i8* 
  %_v0 =  call ccc  %A*  @_A_malloc()  
  %_v1 = getelementptr  %A, %A* %_v0, i32 0, i32 0 
  store   %_A_vtype* @_A_vdata, %_A_vtype** %_v1  
  %_v2 = getelementptr  %A, %A* %_v0, i32 0, i32 1 
  store   i32 0, i32* %_v2  
  %_v3 = getelementptr  %A, %A* %_v0, i32 0, i32 2 
  store   i32 0, i32* %_v3  
  %_v5 = getelementptr  %A, %A* %_v0, i32 0, i32 3 
  store   i8* %_v4, i8** %_v5  
  %_v7 = getelementptr  %A, %A* %_v0, i32 0, i32 4 
  store   i8* %_v6, i8** %_v7  
  %_v8 = alloca %A* 
  store   %A* zeroinitializer, %A** %_v8  
  store   %A* %_v0, %A** %_v8  
  %_v9 = load   %A*, %A** %_v8  
  %_v10 = getelementptr  %A, %A* %_v9, i32 0, i32 1 
  store   i32 42, i32* %_v10  
  %_v11 = load   %A*, %A** %_v8  
  ret %A* %_v11 
}