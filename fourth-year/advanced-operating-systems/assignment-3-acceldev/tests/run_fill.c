...
int main() {
  ...
  for (int i = start; i < start + len / sizeof(uint32_t); i++) {
    if (data_buffer[i] != value) {
      fprintf(stderr, "Different value at %d %u\n", i, data_buffer[i]);
      return 1;
    }
  }
  assert(data_buffer[start + len / sizeof(uint32_t)] == 0);
