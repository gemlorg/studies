...
def equation(points: torch.Tensor) -> torch.Tensor:
    ...
    # Check if all points are within the specified range for all dimensions
    # excluding the last dimension which should be {0}
    
    conditions_met = torch.all((points >= -1) & (points <= 1), dim=1)
    # print(conditions_met)
    
    # Handle the specific exclusion [0, 1) for the last dimension
    # conditions_met &= (points[:, -1] == 0)
    
    # Any point not meeting the condition returns -1, otherwise 1
...
interval = sampler.Interval([-1, -1], [1, 1])
sampler = sampler.Sampler(boundary_path, equation, interval)
...
def main():
    ...
    for epoch in range(epochs+1):
        ...
        plt.scatter(xr[:, 0], xr[:, 1], c='blue', s=0.1)
        plt.scatter(xb[:, 0], xb[:,1], c='red', s=0.1)
        plt.show()
